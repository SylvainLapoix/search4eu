...
We help identify problems and new needs and to address them. All those fixes and new features are integrated in an accessible and free public version of the program.

The philosophy behind it : **building a free and independent tool for the community and by the community**.
...
### The crawlers : our command interface
...
1. it is renamed with a "hash" : as document name format is not standard and naming metadata sometime badly coded, Memorious standardises doc names into a 41 long character non-sense like :
...
2. the format is standardised to ".raw" for handling.

However, the content of the file is intact and it can be displayed easily to check.
...
* by connecting directly to Aleph API with the *aleph_emit* method, available in the *alephclient* package (available [on Github](https://github.com/alephdata/alephclient) and `pip`).
...
### A search engine for health investigation (beta)
